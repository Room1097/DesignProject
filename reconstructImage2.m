function reconstructed_image = reconstructImage2(qx, qy, np, image_size)
    ...
    for p = 0:255
        ...
        for i = 1:num_points
            ...
            x_coord = round(vpa(subs(qx_poly, z, i)));
            y_coord = round(vpa(subs(qy_poly, z, i)));
            fprintf('Evaluating polynomial for p=%d at z=%d x and y \n', p, i);
            disp(x_coord);
            disp(y_coord);

            % Ensure coordinates are within image bounds
            if all([x_coord > 0, x_coord <= image_size, y_coord > 0, y_coord <= image_size])
